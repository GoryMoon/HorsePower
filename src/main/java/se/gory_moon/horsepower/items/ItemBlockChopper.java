...
public class ItemBlockChopper extends ItemBlock {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (!itemstack.isEmpty()
                ...
                && player.canPlayerEdit(posUp, facing, itemstack) && worldIn.mayPlace(ModBlocks.BLOCK_CHOPPER_FILLER, pos, false, facing, null))
        {
            int i = this.getMetadata(itemstack.getMetadata());
            IBlockState blockState = this.block.getStateForPlacement(worldIn, pos, facing, hitX, hitY, hitZ, i, player, hand);
            IBlockState blockStateUp = ModBlocks.BLOCK_CHOPPER_FILLER.getStateForPlacement(worldIn, posUp, facing, hitX, hitY, hitZ, EnumFacing.DOWN.getIndex(), player, hand);
