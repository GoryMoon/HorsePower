...
public class HorsePowerPressCategory extends HorsePowerCategory<PressingRecipe> {
    ...
    public void draw(PressingRecipe recipe, MatrixStack matrixStack, double mouseX, double mouseY) {
        ...
        arrow.draw(matrixStack, isLiquid ? 61: 57, 32);
    }

    @Override
    public List<ITextComponent> getTooltipStrings(PressingRecipe recipe, double mouseX, double mouseY) {
        ...
        if (mouseX >= 55 && mouseY >= 25 && mouseX < 86 && mouseY < 50) {
            ...
            tooltip.add(new TranslationTextComponent("info.horsepower.horse.pressing.time", printLaps));
        }
        return tooltip;
    }
