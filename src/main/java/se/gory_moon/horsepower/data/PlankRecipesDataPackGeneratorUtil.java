...
public class PlankRecipesDataPackGeneratorUtil {
    ...
    private static void generate(String choppingDataPackPath) {
        ...
        Map<RegistryKey<Item>, Item> logs = ForgeRegistries.ITEMS
                .getEntries()
                .stream()
                .filter(e -> e.getValue().getTags().contains(tagLog))
                .collect(Collectors.toMap(Entry::getKey, Entry::getValue));

        //the log to plank map
        Map<Item, Item> logPlankMap = new HashMap<>();
        logs.forEach((key, value) -> {
            ResourceLocation logResourceLocation = key.getLocation();
    ...
    private static String getDataPackPath(MinecraftServer server) {
        ...
        return server.isDedicatedServer() ? (serverPath + "/" + server.func_240776_a_(FolderName.DATAPACKS)): (serverPath + "/saves/" + server.func_240776_a_(FolderName.DATAPACKS));
    }
